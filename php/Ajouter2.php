...
if (!preg_match('/^([0-9A-Fa-f]{2}[:-]){5}([0-9A-Fa-f]{2})$/', $mac)) {
    ...
    echo '<p><input type="button" onclick="window.location.href = \'Ajouter.php\';" class="w3-border w3-button w3-text-white" value="Retour au formulaire"/></p>';
} elseif (!filter_var($ip, FILTER_VALIDATE_IP)) {
    echo '<span style="color:white">Adresse IP invalide. Veuillez recommencer le formulaire.</span>';
    echo '<p><input type="button" onclick="window.location.href = \'Ajouter.php\';" class="w3-border w3-button w3-text-white" value="Retour au formulaire"/></p>';
} else {
    $sql = "INSERT INTO Equipement (nom, adMAC, adIP, CIDR, idT) VALUES ('$nom','$mac','$ip',$cidr,$type)";
    if (mysqli_query($connexion, $sql)) {
        echo '<span style="color:white">Les saisies ont été intégrées à la base de données.</span>';
